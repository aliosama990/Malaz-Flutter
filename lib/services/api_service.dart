...
import '../utils/socket_exception_utils.dart';
  ...
  Future<ApiResponse> _send({
    ...
    try {
      ...
      if (isSocketException(error)) {
        assert(() {
          final details = describeSocketException(error);
          debugPrint(
            '[ApiService] Network error while calling $uri:\n$details',
          );
          return true;
        }());

        throw ApiException('لا يوجد اتصال بالإنترنت', cause: error);
      }
