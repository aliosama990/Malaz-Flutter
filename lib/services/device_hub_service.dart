...
typedef EmergencyAudioAlertCallback = void Function(EmergencyAudioAlert alert);
...
class DeviceHubService {
  ...
  static const List<String> _readingEventNames = <String>[
    'ReceiveDeviceReading',
    'receivedevicereading',
  ];
  ...
  EmergencyAudioAlertCallback? _onEmergencyAudio;
  ...
  Future<void> connectAndSubscribe({
    ...
    _onAlert = onAlert;
    _onEmergencyAudio = onEmergencyAudio;
  ...
  void _registerHandlers(HubConnection connection, int generation) {
    for (final eventName in _readingEventNames) {
    ...
    connection.on('ReceiveEmergencyAudio', (arguments) {
      if (!_isActiveConnection(connection, generation)) {
        return;
      }

      _handleReceiveEmergencyAudio(arguments);
    });
  ...
  void _handleReceiveEmergencyAudio(List<dynamic>? arguments) {
    final alert = EmergencyAudioAlert.tryParse(arguments);
    if (alert == null) {
      return;
    }

    _onEmergencyAudio?.call(alert);
  }
  ...
  Future<void> _stopConnection({
    ...
    if (connection != null) {
      try {
        ...
        connection.off('ReceiveEmergencyAudio');
      } catch (_) {
