...
class DeviceLiveProvider extends ChangeNotifier {
  ...
  static const int _recentEmergencyAudioAlertLimit = 5;
  ...
  EmergencyAudioAlert? get latestEmergencyAudioAlert =>
      _latestEmergencyAudioAlert;
  ...
  List<EmergencyAudioAlert> get recentEmergencyAudioAlerts =>
      List<EmergencyAudioAlert>.unmodifiable(
        _recentEmergencyAudioAlerts,
      );
  ...
  Future<void> connectToDevice(String deviceId) async {
    ...
    try {
      await _deviceHubService.connectAndSubscribe(
        ...
        onEmergencyAudio: (alert) {
          if (!_isCurrentRequest(requestId)) {
            return;
          }

          _handleIncomingEmergencyAudioAlert(alert);
        },
  ...
  void _handleIncomingEmergencyAudioAlert(EmergencyAudioAlert alert) {
    final currentDeviceId = _currentDeviceId;
    final incomingDeviceId = alert.deviceId.trim();
    if (currentDeviceId != null &&
        incomingDeviceId.isNotEmpty &&
    ...
    _latestEmergencyAudioAlert = alert;
    final dedupedAlerts = _recentEmergencyAudioAlerts
        .where((existingAlert) => !_isSameEmergencyAudioAlert(
              existingAlert,
              alert,
            ))
        .toList(growable: false);
    _recentEmergencyAudioAlerts = _pushHistoryItem(
      dedupedAlerts,
      alert,
      _recentEmergencyAudioAlertLimit,
    );
    _notifyListenersSafely();
  }
  ...
  void _clearLiveData() {
    _latestReading = null;
    _latestAlert = null;
    _latestEmergencyAudioAlert = null;
    _recentReadings = const <DeviceReading>[];
    _recentAlerts = const <DeviceAlert>[];
    _recentEmergencyAudioAlerts = const <EmergencyAudioAlert>[];
    _isReadingStale = false;
    _staleReadingTimer?.cancel();
  ...
  bool _isSameEmergencyAudioAlert(
    EmergencyAudioAlert existingAlert,
    EmergencyAudioAlert incomingAlert,
  ) {
    final existingAudioUrl = existingAlert.audioUrl.trim();
    final incomingAudioUrl = incomingAlert.audioUrl.trim();
    if (existingAudioUrl.isNotEmpty && incomingAudioUrl.isNotEmpty) {
      return existingAudioUrl == incomingAudioUrl;
    }

    final existingAudioFileName = existingAlert.audioFileName.trim();
    final incomingAudioFileName = incomingAlert.audioFileName.trim();
    if (existingAudioFileName.isNotEmpty && incomingAudioFileName.isNotEmpty) {
      return existingAudioFileName == incomingAudioFileName;
    }

    return existingAlert.deviceId.trim() == incomingAlert.deviceId.trim() &&
        existingAlert.timestamp == incomingAlert.timestamp;
  }
